...
// Delete medicine if delete button clicked
if (isset($_GET['delete_id']) && $_SESSION['role'] === 'admin') {
    $deleteId = intval($_GET['delete_id']);
    $db->query("DELETE FROM medicines WHERE id = ?", "i", [$deleteId]);
    header("Location: view_medicines.php"); // Refresh page after delete
    exit();
}
...
<nav class="sidebar navbar navbar-expand-lg p-3">
  <ul class="navbar-nav flex-column w-100">
    <?php if ($_SESSION['role'] == 'admin'): ?>
      ...
      <li class="nav-item mb-3">
        <a class="nav-link active disabled" aria-disabled="true" href="view_medicines.php">View Medicine</a>
      </li>
      <li class="nav-item mb-3">
...
<div class="container main-content" style="padding-top: 120px;">
  <h2 class="text-center mb-4 fw-bold">All Medicines</h2>
      ...
      <tbody class="text-center">
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              <?php if ($_SESSION['role'] === 'admin'): ?>
                <a href="view_medicines.php?delete_id=<?php echo $row['id']; ?>" class="btn btn-danger btn-sm"
